...
/**
 *  \file		htdocs/core/menus/standard/oblyon.lib.php
 ...
 * @return	void
 */
function print_oblyon_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0)
{
 ...
 * @return	void
 */
function print_left_oblyon_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='')
{
	global $user,$conf,$langs,$dolibarr_main_db_name,$mysoc;
	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'commercial')
		{
            ...
            if (! empty($conf->commande->enabled))
            {
                ...
                if (! empty($conf->expedition->enabled)) $newmenu->add("/commande/liste.php?leftmenu=orders&viewstatut=2", $langs->trans("StatusOrderOnProcessShort"), 2, $user->rights->commande->lire);
	...
	if (empty($noout))
	{
		...
		for ($i = 0; $i < $num; $i++)
		{
			...
			if ($menu_array[$i]['level'] > 0)
			{
				...
				{
					print '<div class="menu_contenu_disable">'.$tabstring.'<font class="vsmenudisabled vsmenudisabledmargin">'.$menu_array[$i]['titre'].'</font><br></div>'."\n";
				}
			}
		}//end for
		print '</li>'."\n";
